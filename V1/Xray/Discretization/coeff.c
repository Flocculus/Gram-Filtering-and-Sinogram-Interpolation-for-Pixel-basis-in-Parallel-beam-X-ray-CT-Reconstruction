		...
		int nrhs, const mxArray *prhs[])
{
	...
	mwSize SplineDegree = mxGetScalar( prhs[1] );
	
	double *x = mxGetPr( prhs[0] ); // column major (from MATLAB)
	...
	if (SplineDegree > 1) { // otherwise return the input is fine
		SamplesToCoefficients(y, Width, Height, SplineDegree);
	}
